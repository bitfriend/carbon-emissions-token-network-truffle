...
// This is NOT up to date or a real upgrade of CarbonEmissionsTokenNetwork.sol

contract CarbonEmissionsTokenNetworkV2 is Initializable, ERC1155Upgradeable, AccessControlUpgradeable {
    ...
    bytes32 public constant REGISTERED_OFFSET_DEALER = keccak256("REGISTERED_OFFSET_DEALER");
    bytes32 public constant REGISTERED_EMISSIONS_AUDITOR = keccak256("REGISTERED_EMISSIONS_AUDITOR");
    // Consumer role
    ...
    function tokenExists(uint256 tokenId) private view returns (bool) {
        if (_numOfUniqueTokens.current() >= tokenId) {
            return true;
        }
